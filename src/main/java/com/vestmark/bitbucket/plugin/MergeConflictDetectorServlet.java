...
public class MergeConflictDetectorServlet extends HttpServlet {
	...
	@Autowired
	public MergeConflictDetectorServlet(@ComponentImport GitExtendedCommandFactory extendedCmdFactory,
			@ComponentImport AuthenticationContext authenticationContext,
			@ComponentImport SoyTemplateRenderer soyTemplateRenderer, @ComponentImport RefService refService,
			@ComponentImport BranchModelService modelService, @ComponentImport PullRequestService pullRequestService) {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// URL path includes numeric repository and pull request IDs to uniquely
		// identify pull request
		...
		// If the target is not master and is a release branch, find target branch and
		// upstream releases (if any).
		if (!mcd.getToBranchId().equals("refs/heads/master") && toBranchType != null
				&& toBranchType.getId().equals("RELEASE")) {
			Page<Branch> branches = bc.getBranchesByType(toBranchType,
					new PageRequestImpl(0, PageRequestImpl.MAX_PAGE_LIMIT / 2));
			branches.stream().filter(b -> mcd.isRelated(b)).filter(b -> mcd.isUpstreamBranch(b))
					.sorted(branchComparator).forEachOrdered(b -> dryRunMerge(mcd, b));
		}
	...
	private void checkForAutoMergeFailure(MergeConflictDetector mcd) {
		try {
			PullRequestSearchRequest prsReq = new PullRequestSearchRequest.Builder().state(PullRequestState.OPEN)
					.order(PullRequestOrder.NEWEST).fromRepositoryId(mcd.getFromRepo().getId())
					.toRepositoryId(mcd.getToRepo().getId()).build();
			...
			while (begIdx < totalNPRs) {
				...
				boolean amfPRExists = pPRs.stream().anyMatch(r -> r.getTitle().contains("Automatic merge failure")
						&& CollectionUtils.isEmpty(r.getReviewers()));
				if (amfPRExists) {
					mcd.addResult(refService.getDefaultBranch(mcd.getToRepo()), Collections.emptyList(),
							Arrays.asList("Please check for automatic merge failure!"), Collections.emptyList());
					return;
				}
				begIdx += maxNPRs;
			}
	...
	protected void render(HttpServletResponse response, Map<String, Object> data) throws IOException, ServletException {
		...
		try {
			soyTemplateRenderer.render(response.getWriter(), "com.vestmark.bitbucket.automergeconflictdetector:mcd.soy",
					"plugin.mcd.conflictsTab", data);
	...
	public void dryRunMerge(MergeConflictDetector mcd, Branch toBranch) {
		...
		MergeCommandParameters params = new MergeCommandParameters.Builder().dryRun(true).author(mcd.getUser())
				.fromBranch(mcd.getFromBranchId()).toBranch(toBranch.getId())
				.message("MergeConflictDetector dry run merge check.").build();
		try {
			...
			if (e instanceof MergeException) {
				...
				// When a merge cannot be completed automatically, a CommandFailedException is
				// being thrown and caught in this
				...
				// Checking the type of the Exception obj prior to the cast using instanceof did
				// not work.
				// Encasing the cast inside its own try/catch was the only way I could find to
				// keep the plugin from crashing.
				try {
